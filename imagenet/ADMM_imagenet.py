...
learning_rate = [0.0001]; isotropic = [True]; quiet = 0
...
train_data = 'train.csv'; test_data = 'val.csv'; 
top_k = 5; shuffle_train = True; num_samples = 300000; 
num_total_samples = 16*1024; ckpt_write_period_max = 2000; batch_size = 32;
train_file_queue = tf.train.string_input_producer([os.path.join(data_path,train_data) for _ in range(4)], num_epochs=1, shuffle=shuffle_train, seed = 2)
example, label = ADMMutils.ImageProducer_imagenet(train_file_queue,isotropic[model_id]);
...
pred, weight_decay_sum, lr_w1_vars, lr_b1_vars, lr_w2_vars, lr_b2_vars, variables_dict, placeholders_dict, dictADMM, layer_list, layer_type, shape_records = imagenet_models.model_load(model_id,example_batch,phase)
