...
task = args.task; task_set = ['pretrained','ADMM_results']
...
ckp_write_period = 2000;     num_samples = 50000;   batch_size = 50; isotropic = [True]; 
val_file_queue = tf.train.string_input_producer([os.path.join(data_path,test_data)], num_epochs=1, shuffle=False, seed = 2)
example, label = ADMMutils.ImageProducer_imagenet(val_file_queue,isotropic[model_id]);
...
#model_checkpoint = ['Caffenet-1-0-3125-2344']
...
# ==========================================================
phase = 'test';
pred, weight_decay_sum, lr_w1_vars, lr_b1_vars, lr_w2_vars, lr_b2_vars, variables_dict, placeholders_dict, dictTest, layer_list, layer_type, shape_records = imagenet_models.model_load(model_id,example_batch,phase)
...
for chp_num , model_chp in enumerate(model_checkpoint): 
    ...
    for num_layer,id_layer in enumerate(layer_list):
        ...
        solpath['nz'][num_layer][mu_id] = float(np.sum(zero_ind_dict[num_layer])) #np.count_nonzero(bunch_Kernel) / ( blksize[0] * blksize[1] )
